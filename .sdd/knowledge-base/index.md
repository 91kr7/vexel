...
| exclusive-project-needs-no-deps | how-to | test | `--project=exclusive` without `--no-deps` drags the whole suite behind it | `entries/exclusive-project-needs-no-deps.md` |
...
| explain-in-plain-italian | guideline | any | Explanations to the human are in careful Italian and stand without the code: the product and what the operator sees, never the call chain | `entries/explain-in-plain-italian.md` |
