...
**Purpose** → the client-side read surface for the build-cache inventory — what the live channel has
delivered — and the prune it drives.

## Contract

- `useBuildCache(): { records: BuildCacheRecord[], loaded: boolean, error?: string, refresh: () =>
  void, prune }`
  - `records` is what the channel last delivered, and an empty list until it has delivered one.
  ...
  - `prune(): Promise<BuildCachePruneResult>` — a failure propagates to the caller.
...
- **A prune re-reads nothing here**: the server marks the inventory changed as part of the operation,
  so what the operator just did reaches the screen as the push that operation caused
...
- Nothing is re-read on a context switch: the server discards what it holds, says so on the channel,
  and the new context's inventory arrives on it (…-multiplexed_sse/REQ-24).
...
- builders: Builders client (the prune, and `BuildCacheRecord` — the shape the channel delivers)
