...
| BuildCacheService | backend service | `server/src/builders/build-cache-service.ts` | Build-cache inventory in identifier order — deliberately not ranked by size (id, type, size, usage state, recorded build step) — and prune through the CLI channel, reporting the space reclaimed | `specs/build-cache-service.md` |
