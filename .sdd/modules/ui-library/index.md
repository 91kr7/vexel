...
| SessionHeader, SessionEndedOverlay, SessionSurface | UI component | `client/src/ui/terminal/SessionChrome.tsx` | Session header with connection state and disconnect/detach action, session-ended overlay (a plain dim, deliberately not blurred), and the surface that positions one over the other | `specs/session-chrome.md` |
