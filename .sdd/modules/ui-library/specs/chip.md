...
**Purpose** → a short label chip carrying an optional inline secondary action (e.g. "detach"), and a
row of such chips with an optional trailing "add" affordance (e.g. attaching a container to a
network).

## Contract

- `<Chip label actionLabel? onAction? />`
...
- `<ChipGroup items addLabel? onAdd? emptyLabel? />`
  - `items: { key, label, actionLabel?, onAction? }[]` — each rendered as a `Chip`.
...
Shows:
- One chip per item, its own inline action next to its label when given.
