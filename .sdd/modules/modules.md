...
| containers | Container list, lifecycle, inspection/configuration and logs: the Engine API listing/lifecycle/prune/inspect/config service and its bounded CPU/memory sampler, the log-streaming service, their endpoints, the client's containers data clients/hooks, the Containers screen and its detail panel with the logs view. | `containers/index.md` |
