...
| ContextsScreen | UI component | `client/src/contexts/ContextsScreen.tsx` | The Contexts screen: contexts in the object list's comfortable variant (marker, name/kind, endpoint, TLS, description, state), each row revealing its detail with the endpoint in full, create form (local socket / SSH), "Use" and "Remove" in the row's action cluster; the daemon block lives on System & prune | `specs/contexts-screen.md` |
