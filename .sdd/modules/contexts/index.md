...
|-----------|------|------|-----------------|------|
| ContextsService | backend service | `server/src/contexts/contexts-service.ts` | Docker context inventory (name, endpoint, kind, TLS, active) covering every endpoint kind, plus create for the local-socket and SSH kinds, select-active and remove through the CLI channel and the local Docker configuration | `specs/contexts-service.md` |
