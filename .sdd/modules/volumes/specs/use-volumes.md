...
**Purpose** → the client-side read surface for the volume listing: what the live channel has
...
- `useVolumes(): { volumes: VolumeSummary[], loaded: boolean, error?: string, refresh: () => void }`
  - `volumes` is what the channel last delivered, and an empty list until it has delivered one.
...
- **It holds no clock and makes no request**: the listing arrives when the server pushes it, so a
  volume created or removed outside the application reaches the screen with the operator doing
  nothing (…-multiplexed_sse/REQ-17, /REQ-33, /REQ-39).
- **Mounting this hook no longer decides whether the server reads volumes.** It is still mounted by
  `VolumesNetworksScreen` alone, but what keeps the server's demand alive is the open channel, which
  holds every value the refresh cache registers for as long as a window is open
  (…-multiplexed_sse/REQ-13). This is the stated departure from
  `plan-docker_management_app-refresh_cache-client_event_refresh_removal/REQ-40, REQ-41`: volumes are
...
- Volumes client (`VolumeSummary`, the shape the channel delivers)
