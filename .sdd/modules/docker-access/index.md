...
| Docker call log | backend service | `server/src/docker/call-log.ts` | One line written before every call to Docker, on either channel — the Engine API over the socket and the local CLI — with credentials redacted from `argv` and standard input never written | `specs/call-log.md` |
