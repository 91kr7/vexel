...
| ComposeScreen | UI component | `client/src/compose/ComposeScreen.tsx` | The Compose screen: one full-width object list of projects, each row carrying its services as a nested list, with lifecycle/scaling actions in the row's cluster; the selected project's detail panel holds its properties over two views — the compose file editor with validation and confirmed save, and the aggregated per-service logs | `specs/compose-screen.md` |
