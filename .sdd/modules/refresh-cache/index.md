...
|-----------|------|------|-----------------|------|
| Refresh cache | backend service | `server/src/refresh-cache/refresh-cache.ts` | The values the interface asks for repeatedly, held server-side: registration of a kind with its read and period, serving from the held value, one refresher per kind, failure keeping the previous value, marking due from daemon events and from the application's own operations, the demand gate, telling the kinds derived from a value when the value they built on is replaced by a different one, the discard on a context change with the read again it hands the caller it disowned, and the manual reload of every held value | `specs/refresh-cache.md` |
