...
|-----------|------|------|-----------------|------|
| LocalStore | backend service | `server/src/persistence/local-store.ts` | Per-user application-data directory created on first run; namespaced, schema-versioned JSON records with serialized (safe concurrent) writes and atomic read-modify-write updates | `specs/local-store.md` |
