...
| REQ-4 | The detail shows the same tabs in the same order — Logs, Stats, Config, Processes, Inspect and, for a running container, Exec and Attach — opening on the same tab as delivered, with the same data, the same operations, the same confirmations and the same live behaviour. Any observable difference beyond where the detail is drawn and how it is opened and closed is a defect of this change. This is what settled the collision with REQ-18 on 2026-08-26: the property lists losing a column above roughly 1200px is an observable difference, and it is REQ-18's designed width that gave way, not this requirement. |
...
| REQ-18 | The modal is sized by the delivered dialog rules as a large-format surface, alongside the image diff, layer explorer, layer efficiency and filesystem browser dialogs: the card the size of its content with no band of empty glass beside it and nothing rendered outside it, the height bounded by the viewport, and the content scrolling inside the card rather than escaping it. **Amended on 2026-08-26 by the human**: "one designed width" is struck for this surface. The delivered large format is `min(1100px, 92vw)`, where the inline panel this replaces took the frame's width; holding the detail at 1100px cost the operator a property column above roughly 1200px and two at 2560, which REQ-4 makes a defect. The detail's format therefore goes on widening with the viewport. The four other large dialogs keep the width they have — this is a format the detail asks for, not a change to what they render. |
...
| REQ-25 | Operating the Config tab's health-check switch inside the modal does not drag the modal out of position: the switch itself stays inside the viewport, and the modal is not displaced by anything other than its own content growing. **Narrowed on 2026-08-26 by the human**, from "leaves the modal's viewport box unchanged". The clause this requirement was written for is bug-2's focus-scroll drag — a control whose visually hidden input is drawn far from its visible track, carrying its surface off screen when focus lands on it. That is absent here, measured: the hidden input sits 10.3px from its track. What remains is the modal being content-sized and vertically centred, so revealing the five health-check fields grows it 85.2px and raises its top edge by half of that. Any control that reveals fields would do the same; the switch is merely the first. That is what a centred dialog does, not a defect, and the check asserts the switch's presence in the viewport and the absence of a drag rather than an unchanged box. |
