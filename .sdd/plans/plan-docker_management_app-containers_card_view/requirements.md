...
| ID | Requirement |
| --- | --- |
| REQ-59 | The card-row pass of `client/scripts/check-ui-conformance.mjs` is widened to admit the containers card presentation **by name** and by nothing wider: the admission names the containers screen's own file(s) and the library component that carries the card, and admits no pattern that would let any other list through. |
