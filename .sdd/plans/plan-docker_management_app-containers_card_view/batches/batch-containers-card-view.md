...
**Amended 2026-08-25 (second) — a second mock governs the inside of the card.**
`.sdd/analysis/ui-mock/containers-refactor-b3.png` decides where every element sits **within a
card** from that date and supersedes the image above on that point alone; the earlier image is not
deleted and stands as the record of what was originally asked, remaining normative for everything b3
does not redraw. It was chosen by the human from generated working mocks
(`containers-card-layout-variants.png` — whole-card arrangements A/B/C;
`containers-card-ports-variants.png` — port placements B1/B2/B3 on the chosen B), after the delivered
card was judged *"disposti un po' a caso"* and a long image reference was found to wreck the line it
shared. The delivered arrangement is **B3**, and the band order it fixes is **identity → state and
uptime → image → CPU → MEMORY → NET I/O → PORTS → footer actions**. The four faults and their fixes,
the front-truncating image line, the ports' move out of provenance and the deliberately inert detail
control are in the analysis's *Amendment — 2026-08-25 (second): the card's internal arrangement,
chosen on generated mocks*; annotated on REQ-3, REQ-4, REQ-5, REQ-6, REQ-9, REQ-11, REQ-12, REQ-20,
REQ-22, REQ-23, REQ-27, REQ-28, REQ-30, REQ-31 and REQ-34.
...
| INT-3 | modify | `client/src/ui/glass/Surface.tsx` + `client/src/ui/glass/surface.css`, forwarded through `client/src/ui/glass/Card.tsx` | Extend `Surface` with (a) a **selectable/selected treatment** — hover and selected highlights referencing the two tokens the table row already uses — and (b) a **state accent edge**: a full-height bar on the left, following the surface's own left rounding, in a state colour token. `Card` forwards both as optional props. **Amended 2026-08-25 (second)**: a third addition on the same terms — a **footer band**, a slot closing the surface on its own ground under a hairline, the padding moving off the surface and onto its two bands so that ground reaches the edges; it is what takes the actions out of the middle of the card. **Every existing `Surface`/`Card` call site renders exactly what it renders today**, and no `card.css` is created (`specs/card.md`'s "no card stylesheet" invariant stands). This is the ordering the spec fixes, at step 2: extend, do not duplicate. | REQ-2, REQ-18, REQ-28, REQ-29, REQ-30, REQ-31, REQ-33 | — |
...
| INT-7 | create | client, containers area — **`client/src/containers/ContainerCard.tsx`** (this exact path: batch 1 admitted it by name in the conformance guard) | One container as one card, composed from library components only: the extended `Card` carrying the material and the state accent; band 1 — dot · name · uppercase state pill · short monospace id at the left, and at the right the primary lifecycle action, a gap, then the joined `Pause` · `Restart` · `…` cluster (the delivered `ActionButtonGroup` + `Menu`, contract untouched); band 2 — the `image` chip, the ports chip **only when ports are published** (**amended 2026-08-25**: at most three chips, then one `+n` for the remainder, splitting at four so a `+1` is never drawn — the reversal of REQ-5's "none summarised" described above), then the status sentence; band 3 — the metric strip with `CPU`, `MEMORY` and `NET I/O`, their capacity notes, and the *no sample* state where a figure is absent (**amended 2026-08-25**: `stacked`, one metric per row). **Amended 2026-08-25 (second) — the bands are rearranged to `containers-refactor-b3.png`, and the band 1/2/3 reading above is superseded.** Five bands and a footer: **identity** — dot · name at the left (the name ellipsising), the short id anchored at the right and never truncated, and beside it the small square `IconButton` that will open the detail in a modal, **built and deliberately inert** (the human's decision of 2026-08-25: present, not disabled, doing nothing when clicked, and swallowing its own click so it never selects the card — recorded as a decision at the call site, not left looking like a bug); **state and uptime** — the pill beside the daemon's own status sentence, on a line of its own; **image** — a full-width `Chip` field of its own, truncating **at the front**; **metrics** — the stacked strip with `CPU`, `MEMORY`, `NET I/O` and now `PORTS` as one of its track-less rows, reading `none` where the container reports no port; **footer** — the primary lifecycle action at the left and the joined cluster at the right, below a hairline and on their own ground, the footer **pinned to the card's bottom edge** so the stretch slack of a short card opens above the rule and not under it. The wording, the tones and the state rule are unchanged; **the cap is lowered to two chips then a `+n`** (splitting at three), measured: three chips plus the `+n` wrapped the row onto a second line at the delivered track width. The footer's controls take the library's **ordinary** button size and the card its **medium** inset — the mock's proportions read off the nearest existing steps of the scale, no length written here — and the segmented cluster's slots are **one height**, owned by the group (`action-button-group.md`), the delivered 27px/24px mismatch having left its rounded end reading as a bulge. The b3 mock's accented `Stop` is **not** followed — the tone split is a delivered decision and the mock governs arrangement, not tone. Every displayable state gets a pill, an accent and a dot by one rule — `created`, `restarting`, `removing` and `dead` included, not only the mock's three — and the three always agree. No age is displayed. | REQ-2, REQ-3, REQ-4, REQ-5, REQ-6, REQ-7, REQ-8, REQ-9, REQ-11, REQ-12, REQ-13, REQ-14, REQ-16, REQ-18, REQ-19, REQ-20, REQ-22, REQ-27, REQ-31, REQ-34, REQ-35, REQ-53 | INT-1, INT-3, INT-5 |
| INT-8 | modify | `client/src/containers/ContainersScreen.tsx` | Replace the `DataTable` with one `ContainerCard` per matching container and a uniform token gap. **Amended 2026-08-25**: a `Grid arrangement="cards"` — three cards to a row, two at ≤1200px, one below the phone breakpoint — rather than the full-width `Stack` this line originally specified; the `Grid` carries the list's `dismissalFocusTarget`, and the empty state and the detail panel are each wrapped in a `GridSpan` so they take the whole row. **Everything else on this screen is preserved exactly**: the toolbar and its three actions, the search field, the state chips and their filtering (relative order preserved), the empty state, the server's ordering with no sort control and no selection, the inline rename, the per-container action lifecycle and its in-flight disabling, the confirmations, the toasts and every string. The `ContainerDetailPanel` now opens **beneath the selected card**, spanning the whole row of the grid, closing on a second selection and on `Escape`, at most one at a time. **Annotated 2026-08-25 — the panel opens beneath the selected *card*, not beneath its *row*, and that is not being fixed.** Measured at 1440×1000 with four fixtures: selecting the first card of a three-card row pushes its two row-mates from y=367.8 to y=954.8, below the panel at y=623.6, because the row-spanning child is emitted immediately after the owning card and the grid's auto-flow sends the rest of the row past it. The human's decision: the coming intervention that moves the detail into a modal removes this inline panel and its placement with it, so neither the fix nor coverage of it is worth paying for. The e2e check that asserted "beneath the row" was removed for that reason and not because it failed (REQ-23). Live updates land in place: no card moves, the list does not reorder, and nothing is tweened between samples. | REQ-1, REQ-15, REQ-17, REQ-21, REQ-23, REQ-24, REQ-25, REQ-26, REQ-27, REQ-31, REQ-32, REQ-33, REQ-36 | INT-7 |
...
- **Amended 2026-08-25 (second) — the rearrangement's library work, and its one trap.** The front
  ellipsis is a **library** declaration (`direction: rtl` with `text-align: left` on an ellipsised
  single line, written beside the truncation contract), never a style in feature code; the footer's
  ground and hairline are the `Surface`'s; and a name that ellipsises needs every flex box down to it
  allowed to shrink, or the ellipsis never appears at all. The card's own file gains no tag, no
  length and no stylesheet, and the conformance pass is not opened.
