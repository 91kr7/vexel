---
batch: 2
feature: The lists that carry nothing below their cells — builders & build cache, contexts, plugins, swarm nodes, services and secrets — become classic tables
...
**The library is not changed by this batch.** Everything these lists need already exists: the ruled
presentation, and content-sized rows for the ones whose cells carry two lines. If an implementer
...
| --- | --- | --- | --- | --- | --- |
| INT-1 | modify | `client/src/builders/BuildersScreen.tsx` (:362 builders, :389 build cache) | Stop asking for the card presentation on both lists; state content-sized rows only where the row genuinely carries two lines. Name, driver, endpoint, platforms, status, cache size, the active-builder marker, the cache records and every row action with its weight are unchanged. | REQ-2, REQ-3, REQ-4, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13, REQ-16, REQ-33 | — |
| INT-2 | modify | `client/src/contexts/ContextsScreen.tsx` (:260) | The same for the contexts list, whose row carries the active-selection state and the endpoint the row truncates. Name, endpoint, kind, TLS and the active marker keep their values; the switch action keeps its weight and its place. | REQ-2, REQ-3, REQ-4, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13, REQ-17, REQ-33 | — |
| INT-3 | modify | `client/src/plugins/PluginsScreen.tsx` (:318 CLI plugins, :356 managed plugins) | The same for both lists. **This is the named case**: the `WHY UNAVAILABLE` column and its values must share one left edge afterwards. Name, version, availability and the reason keep their values and their columns; no column is added, renamed or reordered to make the point. | REQ-2, REQ-3, REQ-4, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13, REQ-18, REQ-33 | — |
| INT-4 | modify | `client/src/swarm/SwarmNodesPanel.tsx` (:210) | The same for the nodes list: managers first then hostname order, role, availability, state and the removal action unchanged. | REQ-2, REQ-3, REQ-4, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13, REQ-20, REQ-33 | — |
| INT-5 | modify | `client/src/swarm/SwarmServicesPanel.tsx` (:335, :364) | The same for both service lists: name order, image, mode, replicas, ports, the scaling control and the actions unchanged. | REQ-2, REQ-3, REQ-4, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13, REQ-20, REQ-33 | — |
| INT-6 | modify | `client/src/swarm/SwarmSecretsPanel.tsx` (:189) | The same for the secrets list: name order, age, and the write-once treatment of a value that is never read back, unchanged. | REQ-2, REQ-3, REQ-4, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-13, REQ-20, REQ-33 | — |
...
| INT-8 | modify | the criteria check created by `b1/INT-8`, in the client e2e tree | Extend it to these lists — the four screen areas, at the three viewports, with a real pointer — asserting the same four criteria, both lines of any two-line row, one expansion at a time, and the pan at 375×812 with no column at zero. **Plus the named case, asserted as boxes**: on the CLI plugins list at 1440×1000, the `WHY UNAVAILABLE` header's left edge equals its cells', and the delivered drift is on record beside it. Swarm is driven against the stubbed cluster the suite already uses; nothing initialises a swarm on the daemon. | REQ-2, REQ-3, REQ-4, REQ-5, REQ-8, REQ-9, REQ-10, REQ-11, REQ-12, REQ-16, REQ-17, REQ-18, REQ-20, REQ-29, REQ-30, REQ-32, REQ-36 | INT-1 … INT-6 |
...
surfaces and the header-to-body column edge deltas, before and after. And, separately, the Plugins
`WHY UNAVAILABLE` figure — the delivered distance between the value and its header, and the value
after — in px.
