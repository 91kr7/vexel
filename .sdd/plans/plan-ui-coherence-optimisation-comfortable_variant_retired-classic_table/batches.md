...
- **A guard whose premise can go empty is indistinguishable from a guard that passes.** Found twice,
  both times only by *running the thing*. `programme-constraints.test.ts:118` asserts over the
  ...
  degrading the delivered-build comparison from `≠ 16` to `≠ 0`. **The counter-practice extends the
  one already in this file**: a check must be observed failing not only on the rejected build
  (REQ-29) but also **when its subject is absent** — point it at a page, a list or a revision set
  that does not exist and confirm it goes red. `b5/INT-5` re-pins the empty premise and makes the
