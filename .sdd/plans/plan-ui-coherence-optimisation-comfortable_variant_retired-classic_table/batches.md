...
- **`b4/INT-6` is in batch 4 and not in 1, 3 or 6, deliberately.** Not 1 or 3: its subjects are
  ...
  reads *and asserts on*, which is precisely what makes its two complete runs trustworthy as this
  plan's closing evidence — spending that to save a schedule slot is a bad trade. *(The claim was
  first written as "touches no file any test reads" and corrected 2026-08-16: three unit files do
  read `.sdd/modules/ui-library/`. The placement argument is unaffected — a spec restatement there
  would change what a run asserts, which is the thing being protected.)* The spec is therefore **red from
...
- **Test scheduling — one rule, and it holds for every batch.** **No batch runs a full suite.** Each
  ...
  batch 6 — which touches only `.sdd/`, and **no file any test's assertions depend on** (three unit
  files do read `.sdd/modules/ui-library/`, for the blur allow-list, the copy affordance and the
  content-columns contract; batch 6 names them and measures that its edits leave them green) — and
  **batch 6 is not certified until both are green**. That is the only place in this plan where a
...
| REQ-28 | b1/INT-6, b1/INT-7, b1/INT-9, b2/INT-7, b2/INT-9, b3/INT-5, b3/INT-6, b3/INT-7, b4/INT-2, b4/INT-4, b4/INT-6, b4/INT-7, b5/INT-5, b5/INT-6, b5/INT-7, b5/INT-8 | **5** |
...
| REQ-32 | b1/INT-8, b2/INT-8, b3/INT-8, b4/INT-3, b4/INT-4, b4/INT-7 | **4** |
...
| REQ-36 | b1/INT-8, b1/INT-9, b2/INT-8, b3/INT-8, b4/INT-3, b4/INT-6 | **4** |
...
| b4/INT-5 | REQ-27 |
| b4/INT-6 | REQ-28, REQ-36, REQ-40 |
| b4/INT-7 | REQ-28, REQ-32 |
...
- **An enumeration made before the change exists is a starting point, never a perimeter.** Three of
  this plan's lists were found short, each by the work that used them, and each in the same way: they
  were written by grepping for what was *already* nameable. Batch 1's e2e list missed seven specs
  because it was enumerated before REQ-40 existed and so could only find the ones naming the
  presentation; batch 2 found two more of the same class; and batch 6's list of certified batch files
  to annotate missed `batch-4-truncation-contract.md`, having been written the day before the
  retirement existed. **None of the three is a defect** — REQ-25 and REQ-27 already require whatever a
  list missed to be amended, and the mechanism worked all three times. What the plan must not do is
  *treat a list as the boundary of the work*: the requirement is the boundary, the list is a head
  start, and every batch re-derives its own against the tree as it stands.
- **A guard whose premise can go empty is indistinguishable from a guard that passes.** Found three
  times now, and the third by **reading** rather than by running — which matters, because the first
  two made the counter-practice read as "run it and see". `programme-constraints.test.ts:118` asserts over the
  revisions that have touched the conformance script since a pinned base — and that list is **empty**
  on this branch and on `main`, the file's last commit predating the pin, so the guard has been
  asserting nothing while reading as green until a full unit run exposed it. Batch 3's tester found
  the same shape inside this plan's **own new coverage**: a spacing token read on a blank page
  resolved to `0`, quietly turning *"the child is inset by one step"* into *"inset by something"* and
  degrading the delivered-build comparison from `≠ 16` to `≠ 0`. And in
  `library-layer-screens-unmoved.spec.ts`, spotted by inspection: Plugins' `titleOf` resolved every
  empty result to `''` through `closest('.ui-surface')`, so `currentEmpty` became `undefined` and the
  guard *"the empty daemon inventory explains itself"* would have taken its `else` branch and passed
  **with no subject at all**. **The counter-practice extends the one already in this file**: a check
  must be observed failing not only on the rejected build (REQ-29) but also **when its subject is
  absent** — point it at a page, a list or a revision set that does not exist and confirm it goes red
  — and, since the third instance was found by reading, **a fallback branch that can be reached with
  nothing to assert is read for on sight**, not waited for. `b5/INT-5` re-pins the empty premise and makes the
  restated guard fail loudly when it has nothing to check; batches 4 and 5 apply the absent-subject
  test to every check they write.
