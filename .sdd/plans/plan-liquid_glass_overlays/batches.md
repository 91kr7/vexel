...
- **The allow-list is expressed by selector, not by file.** A file-based list could not be exercised
  ...
  matters — *which surfaces* may blur. The six selectors allow-listed in batch 1 are
  `.ui-combobox__list`, `.ui-frame__rail`, `.ui-nav-rail`, `.ui-frame__scrim`,
  `.ui-session-ended-overlay` and `.ui-log-stream__jump`; batch 2 adds the one class its own overlay
  material introduces. The `ui-blur-exception:` marker survives as the residual, commented escape
...
- **The blur goes on the dialog surface, never on the dialog scrim** (REQ-2), by the human's
  ...
  phone drawer is not affected by this: `.ui-frame__rail` and `.ui-frame__scrim` are **siblings**,
  not nested, so both may blur (REQ-5). They do compose, one over the other, for the duration of an
  open drawer on a phone-width viewport; accepted.
