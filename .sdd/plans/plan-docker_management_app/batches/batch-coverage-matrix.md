...
| INT-2 | create | client, coverage feature area | Declaration of the coverage map: one entry per Docker capability area with its coverage state and the screen that covers it, maintained as data so a later batch updates one line rather than a screen. **Four entries are required in state "console only"** — image building (F12, withdrawn 2026-08-07), swarm stack deployment, build-cache export/import and TCP+TLS context creation (all three withdrawn 2026-08-07, departure Three; see "Departures from the spec" in `batches.md`). This matrix is where the product states those gaps honestly, alongside Docker Scout. | REQ-105 | — |
