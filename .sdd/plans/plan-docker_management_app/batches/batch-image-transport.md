...
closed_req: [REQ-42, REQ-43, REQ-116]
depends: [3, 9]
---

# Batch 12 — Image transport: save/load and export/import

**REQ-116 (host-path validation) closes here.** It closed in batch 11 until that batch was withdrawn
on 2026-08-07 (see "Departures from the spec" in `batches.md`); the tarball source and target are now
the first operator-typed host paths in the plan. The validation service itself already exists from
batch 3 — this batch applies it and demonstrates the refusals. Two things the refusal must convey:
the reason (non-existent, wrong kind, unreadable, traversing outside the allowed root) and the fact
that the path is resolved on the machine running the server, which is not necessarily the operator's
own machine.
...
| INT-4 | create | client, data-access layer | Save/load/export/import mutations with their progress streams, host-path validation before starting, and refresh of the image or container list on completion. | REQ-42, REQ-43, REQ-116 | INT-2, INT-3 |
| INT-5 | create | client, images feature area | Save/load and import flows: image multi-selection, target/source path input with refusal messages, progress, and the resulting references reported. | REQ-42, REQ-43, REQ-116 | INT-1, INT-4 |
