...
| REQ-10 | The check's budget stays above the time the daemon takes to refuse, and raising that budget is never the remedy for a missing failure. *(Amended 2026-08-27: this requirement was written granting the forty-five seconds already in place, on the belief that they exceeded the refusal time. Measured on 2026-08-27 over six consecutive runs, this daemon refuses at either 30.1s or 60.2s — one dial attempt or two, about half and half — so forty-five seconds sits between the two modes and breaks this requirement's own first clause on roughly half the runs. The budget is raised to clear the slow path. This is not the remedy the second clause forbids: no failure is missing — the refusal arrives every time, carrying the daemon's address and cause verbatim — the budget was simply tuned to a fast path that is not the only one.)* |
