...
| 15 · about | F16 — About | REQ-70, REQ-71, REQ-72 | 5 | implemented | **Three section-header treatments on one screen become one.** `IDENTITY AND LICENSE` (uppercase, outside a card), `CLI availability` (sentence case, inside a card) and `DAEMON EVENT STREAM` (uppercase, inside a card) now read as the same kind of thing. **Then the event stream is gone from About** — it repeated the Dashboard's verbatim, and About is an identity and licence screen. Confirm the Dashboard's stream still runs, still connects and still shows the same events. **Then confirm nothing of the notice moved**: product name, copyright, licence with a route to each of its two documents, absence of warranty, right to convey, repository with the running version, network-modification duty, reservation of the name — the certified `about_license_notice` plan's whole subject, word for word. |
