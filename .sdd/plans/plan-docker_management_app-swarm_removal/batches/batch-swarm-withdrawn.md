...
| INT-19 | modify | `CLAUDE.md`, `README.md` | The live documents describing the product stop naming a swarm screen. In `CLAUDE.md`: the "Visual reference" list drops `swarm`, and the UI-boundary section's enumeration of feature screens drops it too (`containers/images/volumes/networks/compose/swarm screens`) — it named a screen the application no longer has. In `README.md`: the opening sentence's list of managed areas drops `Swarm`, its punctuation kept readable. Nothing else changes in either file. | REQ-19 | — |
