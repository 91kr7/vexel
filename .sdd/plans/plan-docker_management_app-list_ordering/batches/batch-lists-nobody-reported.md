...
| INT-6 | modify | `client/src/dashboard/DashboardScreen.tsx` — expected to be **no change at all** | The guard rail, verified rather than assumed: the Dashboard's container activity list sorts client-side, grouping by container state before comparing names, and it stays exactly as it is (REQ-42). It reads like the client-side ordering REQ-28 forbids and is not: it is a deliberate grouping, the point of an activity panel. Confirm by `git diff` that the file is untouched, and that the activity list still puts running containers first in the running application. | REQ-42 | — |
