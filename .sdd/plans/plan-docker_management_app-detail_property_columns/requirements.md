...
| REQ-32 | **The copy affordance beside a value is untouched** — its position beside its value, its behaviour and its keyboard reachability. **bug-5 concerns it, is worked next, and nothing here anticipates it either way**: a `Copy` visible in this report's screenshot is not licence to move or remove one. **Discharged 2026-08-14** by [`plan-docker_management_app-remove_copy_controls`](../plan-docker_management_app-remove_copy_controls/requirements.md), which is bug-5: the fence this requirement put around the control did its job, and the control has now been removed by the report it was reserved for. The delivered text is left standing so the fence and its discharge are both on the page. **This report's own arithmetic is not re-opened by that**: the minimum band width derives from `Created`, not from the `Id` band, so no column count moves; the `Id` band merely becomes the height of its neighbours (33px, having been 43px purely because it held the control), and the section becomes marginally shorter, which satisfies this report's height ceilings a fortiori. The four records of this plan that were measured **with the control present** were updated by that plan and by nothing else — see its REQ-24. |
