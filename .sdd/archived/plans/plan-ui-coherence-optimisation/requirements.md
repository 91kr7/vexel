...
**The object list is delivered by extending `DataTable`, not by adding a second list beside it**
(REQ-22). This is what `CLAUDE.md` requires — extend an existing component with a prop or variant
rather than create a near-duplicate, two components that look 90% alike being exactly the divergence
the rule exists to prevent — and it has a consequence that decides the shape of the whole programme:
**the nine migrated screens inherit the F2 column repair by construction**, instead of each acquiring
a column contract of its own. That is what makes F5 a foundation batch rather than an additive one,
and it is why F2 is scheduled before F5 rather than beside it.

**Amended 2026-08-17 — what was delivered here, and what became of it.** This passage read *"by
extending `DataTable` with a comfortable variant"*, and that is what this programme built: the
extension was delivered in batch 5 and nine screens were migrated onto that **comfortable** variant —
each row on a card of its own, under a floating column header. **That variant was retired on
2026-08-16**, prop, carrier surface, stylesheet rules and header-inset compensation together, and its
21 call sites were converted onto the one table presentation containers and images already shipped;
reintroducing it now fails `npm run lint`. The decision that governs is
`plan-ui-coherence-optimisation-comfortable_variant_retired-classic_table` (REQ-1, REQ-22, REQ-25),
on the analysis
[`.sdd/analysis/ui-coherence-optimisation-comfortable_variant_retired-classic_table.md`](../../../analysis/ui-coherence-optimisation-comfortable_variant_retired-classic_table.md).
The decision recorded above — one component extended rather than a second list, and the column repair
inherited by construction — **still holds and is untouched**; what no longer exists is the second
presentation it was carried by. The reason this file is amended rather than left to be read as
history: it is validated, and a validated requirement still demanding the card row is the authority
under which someone reinstates it and is, by the record, correct to do so.
...
| ID | Requirement |
| --- | --- |
| REQ-22 | **One object-list primitive.** One component — not two components, and not a second list beside `DataTable`: **the delivered `DataTable` is extended** (design decision, stated above with its rationale and its consequence — the nine migrated screens inherit the F2 column repair by construction). It provides aligned columns, a header row, per-row actions, an optional inline expansion and the truncation contract of F4. **Amended 2026-08-17**: this requirement read *"with a dense and a comfortable variant … One component with a variant prop"* and ended *"and both variants are available to any screen"*. The comfortable variant — each row on a card of its own under a floating header — **was retired on 2026-08-16**, prop, carrier surface, stylesheet rules and header-inset compensation together, its 21 call sites converted onto the one table presentation, and its reintroduction now fails `npm run lint`. The two-variant clause is struck because it is the authority under which the card row would be reinstated; the primitive, the extension of `DataTable` and the inherited column repair are untouched. Decision: `plan-ui-coherence-optimisation-comfortable_variant_retired-classic_table` (REQ-1, REQ-22, REQ-25), on the analysis `.sdd/analysis/ui-coherence-optimisation-comfortable_variant_retired-classic_table.md`. |
...
| REQ-29 | **Each primitive is covered by its own unit tests**, exercising every variant and every state it offers (tabbed and untabbed; empty state with and without an action; a header with and without a sublabel; each action weight), so that the migrations that follow rest on tested behaviour rather than on their own first use of it. **Amended 2026-08-17**: the enumeration opened with *"dense and comfortable; "*, which obliged coverage of a presentation that no longer exists — the comfortable variant was retired on 2026-08-16 and the unit file dedicated to it went with it, everything in it covering behaviour that survives having been restated against the one presentation. Only that half of the clause is struck: every primitive is still covered by its own unit tests, over every variant and state it does offer. Decision: `plan-ui-coherence-optimisation-comfortable_variant_retired-classic_table` (REQ-25, REQ-28), on the analysis `.sdd/analysis/ui-coherence-optimisation-comfortable_variant_retired-classic_table.md`. |
...
| ID | Requirement |
| --- | --- |
| REQ-81 | **The five questions have exactly one answer each across the whole product.** One way an object is listed (the primitive), one way detail is revealed, one place actions live, one empty-state treatment, one section-header treatment. Counted over the shipped screens at the end of the programme, not asserted per screen: four list paradigms become one, six action affordances become one rule, three toolbar placements become one, three detail-panel shapes become one, three empty-state treatments become one, four section-header styles become one. **Amended 2026-08-17**: the parenthesis read *"(the primitive, in two variants)"*; the second variant — the card row — was retired on 2026-08-16 and every list is now the one table presentation. **The counts are unaffected and stand exactly as written**: they count paradigms, affordances, placements, shapes and treatments, none of which the retirement moves. Decision: `plan-ui-coherence-optimisation-comfortable_variant_retired-classic_table` (REQ-1, REQ-25), on the analysis `.sdd/analysis/ui-coherence-optimisation-comfortable_variant_retired-classic_table.md`. |
