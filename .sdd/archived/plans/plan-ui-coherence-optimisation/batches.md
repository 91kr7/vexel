...
- **`CardList` is absorbed, not merely deprecated.** Its presentation becomes the object list's
  **comfortable** variant (REQ-22) so that seventeen call sites lose nothing in the swap, and the
  ...
  - **Amended 2026-08-17 — the variant that absorbed it was itself retired.** `CardList` was deleted
    in batch 13 as stated, and the **comfortable** variant it had been absorbed into was **retired on
    2026-08-16**, prop, carrier surface, stylesheet rules and header-inset compensation together; its
    21 call sites were converted onto the one table presentation containers and images already
    shipped, and both routes back — asking the library for it, or a feature file drawing a surface
    per row — now fail `npm run lint`. **The absorption still stands as the right call at the time**:
    it is what let seventeen call sites move without losing anything, and it is what made those call
    sites convertible later in one place instead of eleven. What is amended is only the destination:
    there is no second presentation left for a list to be moved onto. Decision:
    `plan-ui-coherence-optimisation-comfortable_variant_retired-classic_table` (REQ-1, REQ-22,
    REQ-25), on `.sdd/analysis/ui-coherence-optimisation-comfortable_variant_retired-classic_table.md`.
