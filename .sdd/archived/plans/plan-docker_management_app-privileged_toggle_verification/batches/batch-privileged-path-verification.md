...
- **The two entry modes.** Containers screen toolbar → `Run container…` (primary action
  ...
  **Images & layers → an image row → `More actions for …` → `Run…`**, which mounts the form from
  ...
  cover. **The image row is new ground, not a re-run**: the human's own investigation used
  `Run container…` and `Create from image…`, both on the containers screen, and never the image
  row's `Run…`. So one of the two modes covered here is a path no reproduction attempt has ever
  taken — worth more than repeating what was already tried, and worth knowing if it is the one that
  fails.
