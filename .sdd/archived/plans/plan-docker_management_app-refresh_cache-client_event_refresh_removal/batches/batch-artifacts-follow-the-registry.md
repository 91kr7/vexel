...
| INT-16 | modify | `client/e2e/container-logs.spec.ts`, `container-stats-processes.spec.ts`, `layer-explorer.spec.ts` | Three sites INT-15 missed, found by the closing run. Two justify serial mode by "another worker's containers appearing mid-interaction": there is one worker and there always has been, and serial mode inside a file could not guard against another one anyway — `container-detail-density.spec.ts` already carries the same reasoning without a worker in it. The third cites a component spec under `images-analysis/`; the module is `image-analysis` and the path does not resolve. | REQ-78 | INT-15 |
