...
| REQ-26 | The raw inspect payload of a container is viewable and copyable as-is. **Amended 2026-08-14 — narrowed to viewable and *selectable* as-is** by [`plan-docker_management_app-remove_copy_controls`](../plan-docker_management_app-remove_copy_controls/requirements.md) (its REQ-23), which removed every copy affordance from the client on the reporter's own one-line instruction. The delivered text is left standing above so that what this requirement used to say is still on the page. **Why this is a withdrawal and not a regression**: the product's own base analysis (`.sdd/analysis/docker_management_app.md`) contains **no copy or clipboard requirement whatsoever** — the affordance entered downstream, at the plan and spec phases — so what is narrowed here was derived downstream, and nothing in the product's analysis is contradicted. The payload itself is unchanged: still shown in full, still exactly as the Engine returned it, still real selectable text in a real scroll area, and still the place a full container id is obtained from — by hand-selection, which that plan's REQ-19 records as an accepted cost. |
