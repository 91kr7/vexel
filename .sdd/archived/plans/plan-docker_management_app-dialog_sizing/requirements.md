...
| REQ-8 | The large-format dialogs — image diff, layer efficiency, layer explorer, filesystem browser — keep the wide format they are entitled to and are not narrowed towards the ordinary width; their designed width is likewise unaltered by this change, and their card is exactly the size of their content, in both directions. **Measured consequence, recorded so it is not read as drift**: the large card moves from 1102px to 1100px (and from 360.8px to 358.8px at a phone width, which is exactly `92vw`). That is the same hairline accounting as REQ-7, one size up — before the fix the width lived on the content, so the fit-content card came out as the designed width *plus* its border; now the card carries `min(1100px, 92vw)` and reads it exactly. Under the rule that the card is the box carrying the designed width, 1100 is the correct figure and 1102 was the off-by-a-border state. It remains unambiguously the wide format: 1100 against 480. |
