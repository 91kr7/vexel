...
| INT-10 | modify | `.sdd/modules/volumes-networks/specs/volumes-panel.md`, `.sdd/modules/volumes-networks/specs/networks-panel.md`, `.sdd/modules/registries/specs/registries-screen.md`, `.sdd/modules/registries/index.md` | Record the one presentation for these lists: how the list is drawn, that the row content is unconditional, that a two-line row is carried by **the reference's own fixed-height row** rather than by content-sized rows (REQ-39), and the surface composition REQ-40 mandates. A spec is what the next implementer reads as current, so it is corrected, not annotated. **Corrected 2026-08-16**: this intervention previously asked the specs to record "that two-line rows are sized by their content", which the amendment reverses. The specs as recorded by the developer are right; this sentence was what was wrong, and it is fixed here so the two do not read as contradicting each other. | REQ-27 | INT-3, INT-4, INT-5 |

**`INT-9`'s file list was incomplete, and the reason matters for batches 2 to 4.** It was enumerated
by grepping for the retired presentation's **name**, and it was written **before REQ-40 existed** — so
it found every spec that names the presentation and none of the specs that merely **assume the old
surface**. Seven further specs scoped a panel as *"the surface containing its heading"*
(`.ui-section-header__title → closest('.ui-surface')` and equivalents), which REQ-40 breaks by moving
the section header out of the card: the `closest` resolves to `null` and the spec fails for a second
reason that has nothing to do with the row. They were restated in this batch and five are green (38
tests); `exclusive/volumes-prune.spec.ts` is restated but not run, the exclusive project being outside
this batch's set, and it is covered by the programme's closing e2e run.

**Batches 2, 3 and 4 therefore enumerate for that class deliberately rather than discovering it three
more times**: grep for the **locator shape** — anything reaching a panel through its heading, or
otherwise assuming the table and its header share one surface — and not only for the presentation's
name. One member of that class is not a locator repair at all and is handled separately, once, at the
end: `client/e2e/library-layer-screens-unmoved.spec.ts`, which asserts what the *previous* programme
delivered on these screens. It is `b4/INT-6`.
