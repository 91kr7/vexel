 ...
 *
 * Every claim here is about **viewport boxes and edges**, content assertions standing beside them
 * and never instead of them, and every interaction is driven with a **real pointer at the visible
 * control's own coordinates** (CLAUDE.md, "What a check drives, and what it measures").
 *
 * The arrangement measured is the one that ships: a **grid of three cards to a row** (two at
 * ≤1200px, one at ≤720px) whose cards **stack their metrics one per row**, with the bands of
 * `containers-refactor-b3.png` inside each.
 *
...
interface ListGeometry {
  ...
  /** Every scrolled ancestor of the grid, and the toolbar above it: what moves a card without moving the list. */
  scrollers: { selector: string; scrollTop: number }[];
  toolbar: Box | null;
...
async function measureListThisFrame(page: Page): Promise<ListGeometry> {
  return await page.evaluate(() => {
    ...
    const cards = Array.from(document.querySelectorAll<HTMLElement>('.ui-frame__content .ui-grid--cards > .ui-surface')).map((card) => {
      ...
      // Chromium serialises the two-position stop as two, so the bar's width is the first non-zero one.
    ...
    return {
      ...
      scrollers: (() => {
        const out: { selector: string; scrollTop: number }[] = [];
        let node = document.querySelector<HTMLElement>('.ui-frame__content .ui-grid--cards')?.parentElement ?? null;
        while (node !== null) {
          if (node.scrollHeight > node.clientHeight + 1) out.push({ selector: node.className || node.tagName, scrollTop: node.scrollTop });
          node = node.parentElement;
        }
        return out;
      })(),
      toolbar: boxOrNull(document.querySelector('.ui-screen-toolbar')),
...
async function openNarrowedTo(page: Page, stem: string, expected: number, viewport = DESKTOP): Promise<void> {
  ...
  // A banner between the toolbar and the grid moves every card 140px down while it stands, and a
 ...
 * (REQ-64, REQ-65). 128s = 25 + 48 + 16 + 25 + 14:
 *
 * - 25s — the fixtures, created here and removed in the `finally`: the file's worst case is twelve
 *   containers;
 * - 48s — the screen opens, narrowed to them and stating what the daemon states: the two 20s waits
 *   `openNarrowedTo` declares, and the 8s of `waitForTheListToCatchUp`, counted at its own
 *   declaration;
 ...
 * - 128s — what the file therefore declares.
 */
test.describe.configure({ timeout: 128_000 });

test.beforeAll(async () => {
  await ensureImage(ALPINE_IMAGE);
});

// REQ-2, REQ-3, REQ-4, REQ-9, REQ-11 — the element map of `containers-refactor-b3.png`, as boxes.
test('the card draws the b3 element map: accent, bands in order, identity at both ends, actions in a footer', async ({ page }) => {
  ...
  try {
    ...
    expect(card.accent!.pointerEvents, 'the accent layer takes a pointer event').toBe('none');
    ...
    expect(
      round(card.footer!.y + card.footer!.height),
      'the footer does not close the card on its bottom edge',
    ).toBeCloseTo(card.box.y + card.box.height - card.border.bottom, 0);
    expect(round(card.footer!.x), 'the footer’s ground stops short of the card’s left edge').toBeCloseTo(card.box.x + card.border.left, 0);
    expect(round(card.footer!.width), 'the footer does not span the card from edge to edge').toBeCloseTo(
      card.box.width - card.border.left - card.border.right,
      0,
    );
    ...
    expect(card.detailControl!.label, 'the detail control carries no accessible name of its own').toBe(`Open ${fixtures.running} details`);
    ...
    expect(card.pillText, 'the state pill does not read the state in upper case').toBe('RUNNING');
    ...
    expect(card.status!.text, 'the daemon’s own status sentence is not passed through').toMatch(/^Up /);
    ...
    expect(card.image!.text, 'the image field does not state the reference').toContain('alpine:3.20');
    expect(round(card.image!.box.x), 'the image field does not start at the card’s inner left edge').toBeCloseTo(card.inner.left, 0);
    expect(round(card.image!.box.x + card.image!.box.width), 'the image field does not span the card’s inner width').toBeCloseTo(
      card.inner.right,
      0,
    );
    expect(round(card.image!.box.y), 'the image is not the third band').toBeCloseTo(card.bands[2].y, 0);
    ...
    expect(primary.label, 'the primary slot does not carry the state-appropriate action').toBe('Stop');
...
// REQ-5, REQ-6, REQ-7, REQ-8 — the stacked strip: one metric per row, `NET I/O` and `PORTS` included.
test('the metric strip stacks one metric per row, each spanning the card and ending at its own right edge', async ({ page }) => {
  ...
  try {
    ...
    const list = await measureList(page);
    const card = list.cards[0];
    expect(card.columns, 'the strip does not draw CPU, MEMORY and NET I/O').toHaveLength(3);
    ...
    for (const column of [cpu, memory]) {
      expect(column.labelBox!.x, `${column.label}: the label does not start at its row’s left edge`).toBeCloseTo(column.box.x, 0);
      ...
      expect(column.track!.width, `${column.label}: the track does not span its row`).toBeCloseTo(column.box.width, 0);
    ...
    expect(netio.readings[0].box.x, 'the NET I/O readings are not drawn `in` then `out`').toBeLessThan(netio.readings[1].box.x);
    expect(
    ...
    expect(ports.chips[0].text, 'the published mapping is not worded public→private').toMatch(/\d+→5432/);
...
test('the list is a grid of three cards to a row, equal in a row and aligned down a column', async ({ page }) => {
  ...
  try {
    ...
    const list = await measureList(page);
    expect(list.cards.map((card) => card.name), 'the list is not in the served order').toEqual(names);
    ...
    expect(many.ports!.chips.map((chip) => chip.text), 'a card with four ports does not draw two chips and a count').toHaveLength(3);
...
test('the grid drops to two tracks at 1200px and to one at the phone breakpoint', async ({ page }) => {
  ...
  try {
    ...
    for (let index = 1; index < phone.cards.length; index += 1) {
      ...
      expect(round(phone.cards[index].box.x), 'the cards do not share one left edge at the phone breakpoint').toBeCloseTo(
        phone.cards[0].box.x,
        0,
      );
    }
...
// REQ-13, REQ-16 — a metric with no sample against a measured one, told apart on sight.
test('a card with no sample is drawn unlike a measured one, and unlike a measured zero', async ({ page }) => {
  ...
  try {
    ...
    for (const column of unmeasured.columns.slice(0, 2)) {
      ...
      expect(column.track!.width, `${column.label}: the unmeasured track is omitted rather than drawn empty`).toBeGreaterThan(0);
    }
...
// REQ-18, REQ-19 — one state per card, agreed by the accent, the dot, the pill and the fills, and
// told apart from the others by the colour each is painted.
...
// REQ-15, REQ-17, REQ-36 — a live update lands in place, and the ports hold still while it does.
...
// REQ-32, REQ-33, REQ-35 — what the presentation costs, and a measured scroll at a realistic count.
...
// REQ-34 — the same map at 375×812, carrying the same values and clipping nothing.
test('at 375×812 the card reflows and carries the same values as at desktop width', async ({ page }) => {
  ...
  try {
    ...
    expect(narrow.bands, 'the phone card draws a different set of bands').toHaveLength(wide.bands.length);
    expect(narrow.columns, 'the phone card draws a reduced metric set').toHaveLength(3);
    for (let index = 1; index < narrow.columns.length; index += 1) {
      ...
      expect(round(narrow.columns[index].box.x), 'the stacked rows do not share one left edge').toBeCloseTo(narrow.columns[0].box.x, 0);
      expect(round(narrow.columns[index].box.width), 'the stacked rows are not of one width').toBeCloseTo(
        narrow.columns[0].box.width,
        0,
      );
    }
...
test('the detail control keeps its geometry and opens the detail, the card staying where it was and unmarked', async ({ page }) => {
  ...
  try {
    ...
    expect(round(card.detailControl!.box.width), 'the detail control is not the square box the scale gives it').toBe(
